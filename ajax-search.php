...
else
{
    ...
    ?>
            ...
            <a href='#' onclick='myFunction()' name="isbn" value=" <?php echo $isbn;?>"><?php echo '<img src="./couverture/'.$img.'" style="width:75px;height:75px;padding:0px;border:1px;"/>'; ?></a>
           </fieldset>
            <div style="position:relative">
            </br>
            
            <a href='#' onclick='myFunction()' name="isbn" value=" <?php echo $isbn;?>" > <?php echo '<h3><p>'.( $titre ).'</p></h3>'; ?></a>
            </form>
            ...
            <?php if(isset($post['DATE_PARUTION'])){
              ?><p class="date">Date de parution:<?php echo date("d-m-Y", strtotime($post['DATE_PARUTION'])); ?></p><?php
            }?>
            ...
            ?>
            <p class="prixqte"><?php echo "prix:".$post['PRIX']."€ Qté:"; if($post['QUANTITE_STOCK'] != ''){echo $post['QUANTITE_STOCK'];}else echo '0'; ?></p>
            <?php if($post['RESUME'] != ''){
              ?><p class="resume">
                <?php echo $post['RESUME']; ?>  
              </p><?php }
              if($post['NOTE_GERANT'] != ''){
                ?><p class="note">Note du gérant: <?php echo $post['NOTE_GERANT'];} ?></p>
                <?php
              if($post['MOTCLES'] != ''){
                ?><p class="motcles">Mots-clés: <?php echo $post['MOTCLES'];} ?></p>
        </div>
        <hr/>
    <?php
    }
